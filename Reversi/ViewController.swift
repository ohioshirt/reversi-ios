...
extension ViewController {
    ...
    func saveGame() throws {
        let darkPlayMode: Player = playerControls[0].selectedSegmentIndex.description == String(Player.computer.rawValue)
            ? .computer
            : .manual
        let lightPlayMode: Player = playerControls[1].selectedSegmentIndex.description == String(Player.computer.rawValue)
            ? .computer
            : .manual
        let boardDisks = boardView.yRange.map { y in
            boardView.xRange.map{ x in boardView.diskAt(x: x, y: y) }
        }
        let gameState = GameState(diskTurn: turn,
                                  playerDarkPlayMode: darkPlayMode,
                                  playerLightPlayMode: lightPlayMode,
                                  disksOnBoard: boardDisks)

        try? GameStateStore.save(gameState)
    }
    
    /// ゲームの状態をファイルから読み込み、復元します。
    func loadGame() throws {
        guard let gameState = try? GameStateStore.load() else {
            // File broken?
            return
        }
        
        // restore turn
        turn = gameState.diskTurn
        
        // set SegmentedControl
        playerControls[Disk.sides.firstIndex(of: Disk.dark)!].selectedSegmentIndex = gameState.playerDarkPlayMode.rawValue
        playerControls[Disk.sides.firstIndex(of: Disk.light)!].selectedSegmentIndex = gameState.playerLightPlayMode.rawValue
        
        // setDisk
        var i = 0
        for y in 0 ..< Board.height {
            for x in 0 ..< Board.width {
                if let disk = gameState.disksOnBoard[y][x] {
